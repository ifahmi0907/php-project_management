...
if (isset($_GET['proses']))
{
	...
	if ($proses=='tambah_proyek')
	{
		...
		$no_spk = "$no_proyek/SPK/TR.39.52/$singkatan/$tahun";
		
		$ppn = $biaya * 0.1;
		$nilai_proyek = ($biaya + $ppn);
		...
		else 
		{
		   ...
		   $input = "INSERT INTO proyek 
		            (id_proyek, username, id_jenis, no_spk, nama_proyek, tanggal_mulai, tanggal_selesai, durasi_proyek, biaya, nilai_proyek, pemilik, id_prov, id_kabkot, id_kec)
					VALUES
				    ('$id_proyek','$username','$id_jenis','$no_spk','$nama_proyek','$tanggal_mulai','$tanggal_selesai',
				    '$durasi_proyek','$biaya','$np','$pemilik','$id_prov','$id_kabkot', '$id_kec')";
